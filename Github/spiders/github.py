...
class GithubSpider(scrapy.Spider):
    ...
    def get_repos(self, u, *, start=1, stop=3):
        repos = []
        for p in range(start, stop+1):
            url = u + '?page='+str(p)+'?tab=repositories'
    ...
    def parse(self, response):
        ...
        repositories = nav.xpath('./a[2]/span/text()')[0].extract().split()[0]
        if repositories[-1] == 'k':
            repositories = float(repositories[:-1])*1000
        item['repositories'] = int(repositories)

        stars = nav.xpath('./a[4]/span/text()')[0].extract().split()[0]
        if stars[-1] == 'k':
            stars = float(stars[:-1])*1000
        item['stars'] = int(stars)

        followers = nav.xpath('./a[5]/span/text()')[0].extract().split()[0]
        if followers[-1] == 'k':
            followers = float(followers[:-1])*1000
        item['followers'] = int(followers)

        followings = nav.xpath('./a[6]/span/text()')[0].extract().split()[0]
        if followings[-1] == 'k':
